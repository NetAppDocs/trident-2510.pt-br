...
summary: São necessárias várias instâncias do Trident quando se deseja ter várias configurações de armazenamento disponíveis simultaneamente. A chave para múltiplas instâncias é dar nomes diferentes a elas usando a opção --alias com o plugin containerizado ou a opção --volume-driver ao instanciar Trident no host. 
---
